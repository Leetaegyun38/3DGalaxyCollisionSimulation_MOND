...
def fsmall(t, r):#별이 두 블랙홀에 받는 힘
    ...
    rc0 = np.sqrt(x**2 + y**2 + z**2) #원점 거리
    rc1 = np.sqrt((x-GX1)**2 + (y-GY1)**2 + (z-GZ1)**2) #두번째 은하 거리
    ...
    xddot = -x/rc0*np.sqrt(C*GM0*A0/rc0**2) -(x-GX1)/rc1*np.sqrt(C*GM0*A0/rc1**2)
    yddot = -y/rc0*np.sqrt(C*GM0*A0/rc0**2) -(y-GY1)/rc1*np.sqrt(C*GM0*A0/rc1**2)
    zddot = -z/rc0*np.sqrt(C*GM0*A0/rc0**2) -(z-GZ1)/rc1*np.sqrt(C*GM0*A0/rc1**2)
...
def fbig(t, r): #두번째은하 블랙홀이 첫번째 은하 블랙홀에 받는 힘
    ...
    Rc = np.sqrt(GX1**2 + GY1**2 + GZ1**2) #두 은하 사이 거리
    ...
    xddot = -GX1/Rc*np.sqrt(C*GM0*A0/Rc**2)
    yddot = -GY1/Rc*np.sqrt(C*GM0*A0/Rc**2)
    zddot = -GZ1/Rc*np.sqrt(C*GM0*A0/Rc**2)
...
for k in tqdm(range(len(pt))):
    ...
    # draw GALAXY1
    ax.scatter(R0[0], R0[1], R0[2], color='lightcoral', marker='o', s=10, \
    ...
    # draw GALAXY2
    ax.scatter(pG[k,0], pG[k,1], pG[k,2], color='lightsteelblue', marker='o', s=7, \
    ...
    #ax = Axes3D(fig)
    X = np.arange(ax_min,ax_max*1.15,15)
    Y = np.arange(ax_min,ax_max*1.15,15)
    ...
    ax = fig.add_subplot(222,aspect=graph_ratio)
    ax.plot(0,0, color='lightcoral', marker='o', ms=5, label='M1 = %5.1E M_sun' % (M[0],)) 
    ax.plot(np.sqrt(pG[k,0]**2+pG[k,1]**2+pG[k,2]**2), \
        np.sqrt(pG[k,3]**2+pG[k,4]**2+pG[k,5]**2)*3.08567758128e+16/3.1556952e+13, \
        color='lightsteelblue', marker='o', ms=5, label='M2 = %5.1E M_sun' % (M[1],)) 
    ...
    for j in range(N_TOT):
        if j < N_TOT*0.5:
            pdot, = ax.plot(np.sqrt(px[k,j]**2+py[k,j]**2+pz[k,j]**2), \
                np.sqrt(pvx[k,j]**2+pvy[k,j]**2+pvz[k,j]**2)*3.08567758128e+16/3.1556952e+13, 'ro', ms=0.6, \
                    markeredgewidth=1, alpha=1)
        else:
            pdot, = ax.plot(np.sqrt(px[k,j]**2+py[k,j]**2+pz[k,j]**2), \
                np.sqrt(pvx[k,j]**2+pvy[k,j]**2+pvz[k,j]**2)*3.08567758128e+16/3.1556952e+13, 'bo', ms=0.6, \
                    markeredgewidth=1, alpha=1)
    pdots.append(pdot)
    ...
    plt.xlim(0,radius_ax_max)
    plt.ylim(0,velocity_ax_max)
    #plt.legend(fontsize=6,loc='upper right', bbox_to_anchor=(1,1.25))
    plt.draw()

    ax = fig.add_subplot(224,aspect=graph_ratio)
    ax.plot(0,0, color='lightsteelblue', marker='o', ms=5, ls = '', label='M2 = %5.1E M_sun' % (M[1],)) 
    ax.plot(np.sqrt(pG[k,0]**2+pG[k,1]**2+pG[k,2]**2), np.sqrt(pG[k,3]**2+pG[k,4]**2+pG[k,5]**2)*3.08567758128e+16/3.1556952e+13, \
        color='lightcoral', marker='o', ms=5, ls = '', label='M1 = %5.1E M_sun' % (M[0],))
    #labe1= 'velocity'
    pdots = [] 
    for j in range(N_TOT):
        if j < N_TOT*0.5:
            pdot, = ax.plot(np.sqrt((px[k,j]-pG[k,0])**2+(py[k,j]-pG[k,1])**2+(pz[k,j]-pG[k,2])**2), \
                np.sqrt((pvx[k,j]-pG[k,3])**2+(pvy[k,j]-pG[k,4])**2+(pvz[k,j]-pG[k,5])**2)*3.08567758128e+16/3.1556952e+13, 'ro', ms=0.6, \
                    markeredgewidth=1, alpha=1)
        else:
            pdot, = ax.plot(np.sqrt((px[k,j]-pG[k,0])**2+(py[k,j]-pG[k,1])**2+(pz[k,j]-pG[k,2])**2), \
                np.sqrt((pvx[k,j]-pG[k,3])**2+(pvy[k,j]-pG[k,4])**2+(pvz[k,j]-pG[k,5])**2)*3.08567758128e+16/3.1556952e+13, 'bo', ms=0.6, \
                    markeredgewidth=1, alpha=1)
    pdots.append(pdot)
    tlabe1 = ax.text(1.2e1,2e-1,'  ', fontsize=20)
    ax.set_xlabel('R[kpc]')
    ...
    plt.xlim(0,radius_ax_max)
    plt.ylim(0,velocity_ax_max)
    plt.legend(fontsize=6,loc='lower left', bbox_to_anchor=(label_location, 0))
    plt.draw()
    ...
    plt.savefig(r'D:\galaxy_data\mond\pic-%05i' % (k,), dpi=300)
    plt.close()
...
### 이미지 gif 변환
directory = r'D:\galaxy_data\mond'
file_type = r'png'
save_gif_name = r'Galactic_interation_allinone'
time = 20/len(pt)
