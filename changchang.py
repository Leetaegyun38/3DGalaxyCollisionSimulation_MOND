...
for k in tqdm(range(len(pt))):
    ...
    plt.savefig(r'C:\Users\enter1\Desktop\chang\galaxychang\interaction_image\pic%05d.png'%k, dpi=200)
    plt.close()


    ### velocity
    fig = plt.figure(2)
    ax = fig.add_subplot(211,aspect=50)
    ax.plot(np.sqrt(pG[k,0]**2+pG[k,1]**2+pG[k,2]**2), np.sqrt(pG[k,3]**2+pG[k,4]**2+pG[k,5]**2), 'go', ms=10, \
        label='M2 = %5.1E M_sun' % (M[1],)) 
    #labe1= 'velocity'
    pdots = [] 
    ...
    ax = fig.add_subplot(212,aspect=50)
    ax.plot(np.sqrt(pG[k,0]**2+pG[k,1]**2+pG[k,2]**2), np.sqrt(pG[k,3]**2+pG[k,4]**2+pG[k,5]**2), 'go', ms=10, \
    ...
    plt.savefig(r'C:\Users\enter1\Desktop\chang\galaxychang\interaction_velocity\velocity-%05i' % (k,))

print("Making gif file", "Please Wait...")


### 이미지 gif 변환
directory = r'C:\Users\enter1\Desktop\chang\galaxychang\interaction_image'
file_type = r'png'
save_gif_name = r'Galactic_interation_3D'
time = 20/len(pt)
...
directory = r'C:\Users\enter1\Desktop\chang\galaxychang\interaction_velocity'
file_type = r'png'
